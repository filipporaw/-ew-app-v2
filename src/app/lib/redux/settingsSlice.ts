...
export type GeneralSetting = Exclude<
  keyof Settings,
  "formToShow" | "formToHeading" | "formsOrder" | "showBulletPoints" | "privacyStatements"
>;
...
export const initialSettings: Settings = {
  ...
  sectionHeadingColor: DEFAULT_FONT_COLOR,
...
export const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    changePrivacyStatement: (
      draft,
      action: PayloadAction<{
        field: "italyPrivacy" | "euPrivacy";
        value: boolean;
      }>
    ) => {
      const { field, value } = action.payload;
      draft.privacyStatements[field] = value;
    },
...
export const selectPrivacyStatements = (state: RootState) =>
  state.settings.privacyStatements;
