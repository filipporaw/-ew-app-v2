...
export const ElegantSection = ({
  ...
  // light band like the screenshot
  return (
    <View style={{ ...styles.flexCol, gap: spacing["2"], marginTop: spacing["5"], ...style }}>
      <View
        style={{
          width: spacing["full"],
...
export const ElegantBulletList = ({
  items,
    ...
    <View style={{ ...styles.flexCol, gap: spacing["1"], width: "100%" }}>
      {cleaned.map((item, idx) => (
        <View key={idx} style={{ ...styles.flexRow, flexWrap: "nowrap", width: "100%" }}>
          {showBulletPoints ? (
            <Text style={{ width: spacing["4"], lineHeight: 0.6, flexShrink: 0 }}>
              {"•"}
            </Text>
          ) : null}
          <Text style={{ flex: 1, lineHeight: 0.6 }}>
            {item}
          </Text>
        </View>
      ))}
