...
import { getTheme, getAllThemesAvailable } from "components/themes/lib";
...
export const ResumePDF = ({
  ...
  console.log(`[ResumePDF] Rendering with theme: "${theme}", isPDF: ${isPDF}, available themes: ${getAllThemesAvailable().join(", ")}`);

  const ThemeComponent = getTheme(theme);

  if (!ThemeComponent) {
    console.error(`[ResumePDF] ThemeComponent is undefined for theme: "${theme}"`);
    return null;
  }
