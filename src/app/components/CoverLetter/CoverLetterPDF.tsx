...
import { CoverLetterRoverPDF } from "./CoverLetterRoverPDF";
import { CoverLetterRoadPDF } from "./CoverLetterRoadPDF";
import { CoverLetterElegantPDF } from "./CoverLetterElegantPDF";
...
// ... existing createMinimalStyles remains for now or I could move it to its own file ...

const createMinimalStyles = (
  settings: any,
  ...
  return StyleSheet.create({
    ...
    paragraph: {
      ...
      textAlign: 'justify',
      marginBottom: 12 * spacingMultiplier, // Allineato a minimal HTML
      minHeight: (fontSizeValue - 0.5) * 1.6,
    },
    closing: {
      fontSize: fontSizeValue,
      fontWeight: 'medium',
...
export const CoverLetterPDF = ({ coverLetter, settings, isPDF }: CoverLetterPDFProps) => {
  ...
  if (coverLetterTheme === "rover") {
    return <CoverLetterRoverPDF coverLetter={coverLetter} settings={settings} />;
  }

  if (coverLetterTheme === "road") {
    return <CoverLetterRoadPDF coverLetter={coverLetter} settings={settings} />;
  }

  if (coverLetterTheme === "elegant") {
    return <CoverLetterElegantPDF coverLetter={coverLetter} settings={settings} />;
  }
  
  // Tema minimal (kept as fallback for "minimal" or others)
  ...
  const styles = createMinimalStyles(settings, pageMarginStyle);
  ...
  const coverLetterData = {
    personal: {
      ...
      email: profile.email,
      phone: profile.phone,
      website: profile.location || "filippo---raw.com"
    },
    company: {
